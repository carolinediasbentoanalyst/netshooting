<#
.SYNOPSIS
  Troubleshooting de rede para Windows Server 2016 a 2025 (PowerShell) - NÃO interativo.
...
.DESCRIPTION
  ...
  Não solicita entrada do usuário. Usa padrões se não forem fornecidos por parâmetro.
...
.PARAMETER PathPingTarget
  Destino para PathPing (padrão: 8.8.8.8)
...
if (-not $DnsNames -or $DnsNames.Count -eq 0) { $DnsNames = $defaultDnsNames }
if ([string]::IsNullOrWhiteSpace($PathPingTarget)) { $PathPingTarget = "8.8.8.8" }
...
function Step {
  ...
  Write-Log ("ETAPA {0}/{1}: {2}" -f $Index, $Total, $Title)
  Write-Progress -Activity "Network Troubleshoot" -Status $Title -PercentComplete $pct
}
...
function Invoke-PathPingWithProgress {
  ...
  Write-Host ("PathPing em execução por até {0} minutos (destino: {1})..." -f ([Math]::Round($DurationSeconds/60,2)), $Target) -ForegroundColor Cyan
  ...
  while (-not $p.HasExited -and $sw.Elapsed.TotalSeconds -lt $DurationSeconds) {
    ...
    Write-Host ("  PathPing... {0}s / {1}s" -f $elapsed, $DurationSeconds)
  }
...
# --------------------------
# Pipeline de execução (etapas)
# --------------------------
$TOTAL = 8
$step = 0

Step -Index (++$step) -Total $TOTAL -Title "Setup de pastas e log"
...
Step -Index (++$step) -Total $TOTAL -Title "Inicialização do relatório"
$report = [ordered]@{
  meta = [ordered]@{
...
Step -Index (++$step) -Total $TOTAL -Title "Coletando IP público (ipify)"
...
Step -Index (++$step) -Total $TOTAL -Title "Coletando informações do sistema operacional (CIM)"
try {
...
Step -Index (++$step) -Total $TOTAL -Title "Coletando snapshots RAW (netsh IPv4/IPv6)"
...
Step -Index (++$step) -Total $TOTAL -Title "Coletando informações estruturadas de rede (NetTCP/IP + DNS)"
try {
...
Step -Index (++$step) -Total $TOTAL -Title "Executando testes (ICMP/Traceroute/DNS/TCP/PathPing)"
# ICMP + Traceroute
foreach ($t in $Targets) {
  ...
  $icmpObj = [ordered]@{ target = $tTrim; success = $false; details = $null; error = $null }
  ...
  $report.tests.traceroute += [pscustomobject]@{ target = $tTrim; outputFile = (Split-Path -Leaf $trFile) }
}
...
foreach ($name in $DnsNames) {
  ...
  $dnsObj = [ordered]@{ name = $nTrim; success = $false; answers = $null; error = $null }
...
# Finalize
# --------------------------
Step -Index (++$step) -Total $TOTAL -Title "Gerando SUMMARY + JSON (e ZIP opcional)"
...
Write-Progress -Activity "Network Troubleshoot" -Completed -Status "Concluído"
